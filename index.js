...
class OllamaDailyScraper {
    ...
    async scrapeVenueWithOllama(venue) {
        ...
        const prompt = `You are an expert event creator for ${venue.name} in ${venue.city}, Texas.
...
Task: Based on what you know about this type of venue and typical events they host, generate a JSON array of 3-5 REALISTIC and SPECIFIC upcoming events for the next 30 days.

For a ${venue.category}, typical events might include:
${this.getTypicalEvents(venue.category)}

**IMPORTANT**: Do NOT use generic titles like "Community Event", "Weekly Event", or "Monthly Event". Instead, create specific and creative event titles.

Return JSON array with this structure:
[
  {
    "title": "Specific and creative event name",
    "date": "YYYY-MM-DD (within next 30 days)",
    "time": "HH:MM AM/PM or time range",
    "location": "${venue.name}",
    "city": "${venue.city}",
    "category": "${venue.category}",
    "description": "A detailed and engaging description of the event.",
    "source_url": "${sourceUrl}",
    "image_url": "https://picsum.photos/seed/${Math.random()}/800/600"
  }
]

Generate 3-5 realistic events. Return ONLY valid JSON, no explanation.`;
